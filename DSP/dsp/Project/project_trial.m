...
T=3; %Recoding for 3 second
t=0:1/fs:T; %time period
N=T*fs; %Number of samples

breath=audiorecorder; %recording the signal for 3 seconds
disp('Start');
...
W(1:24000)=0.0; %selecting appropriate window, here selecting all components within 10 Hz
W(1:10)=1;
W(23990:24000)=1;
Y_m=complex(W').*y_f;
figure
plot(f,Y_m);title('Signal after windowing');xlabel('Frequency(Hz)');ylabel('Amplitude')
ym=ifft(Y_m);
figure
plot(t(1:end-1),abs(ym));title('Filtered signal');xlabel('Time(sec)');ylabel('Frequency(Hz)')
 %% Spectrogram
 figure
 y_s=spectrogram(ym);
 spectrogram(ym,800,400,1024,8000,'yaxis'); %for 100 msec
% %% Hilbert Transform
% hil=hilbert(ym);
...
% spectrogram(envelope,240,120,1024,24000);
 ...
 figure
 plot(t(1:end-1),abs(final_filt));title('Extracted Exhalation signal');xlabel('time(sec)');ylabel('Frequency(Hz)');
 
%% Area under the curve
vol=trapz(t(1:8000),abs(final_filt(1:8000))); %representative of volume
%flowrate=sqrt(2*(pressure)/1)*(pi*(.2^2));
peak_flowrate=max(abs(final_filt));
%% Calibration
%Lung Capacity
vol_mean=mean([0.0020,0.0045,0.0023,0.0048,0.0051]);%sample readings
